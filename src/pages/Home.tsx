  ...
  return (
    <>
      <section className="flex flex-col items-start gap-10 py-8 md:flex-row md:items-center md:justify-between md:gap-16">
        ...
        <HeroIllustration className="hidden w-full max-w-xs shrink-0 text-neutral-300 dark:text-neutral-700 sm:block md:w-80" />
      </section>

      <section className="mt-8 border-t border-neutral-200 py-14 dark:border-neutral-800">
        <div className="grid gap-10 sm:grid-cols-3">
          {FEATURES.map(({ Icon, title, body }) => (
            <div key={title} className="flex flex-col gap-3">
              <Icon className="h-8 w-8 text-accent" />
              <h2 className="text-lg font-semibold">{title}</h2>
              <p className="text-neutral-500 dark:text-neutral-400">{body}</p>
            </div>
          ))}
        </div>
        <Link
          to="/articles/design-principles"
          className="mt-10 inline-block text-sm text-accent no-underline hover:text-accent-hover"
        >
          See all 30 rules this site follows &rarr;
        </Link>
      </section>
    </>
  );
