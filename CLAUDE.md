...
**Live-demo rule**: whenever a `DocArticle` explains CSS behavior that can be shown running in a browser, include a `demo` section demonstrating it, not just prose/`ascii`/`code` description. This is a standing rule — apply it every time, in every article, not just new ones; prose is for explaining *why*, the `demo` section is for showing the actual behavior.

**Syntax-section rule**: every CSS Docs page (`src/content/css-docs/`) opens with a `{ kind: "heading", text: "Syntax" }` + `css` code block right after its intro paragraph — the property/function's value forms for a property page, or the condensed recipe for a technique page. Apply it to every new CSS doc.

**Inline emphasis in prose**: all prose strings in the content model (paragraphs, list items, callout text, table cells, comparison points, captions, rule bodies) pass through `renderInline` (`src/components/inline.tsx`), a mini markup parser: `**bold**`, `` `inline code` `` (bordered mono chip), `==highlight==` (translucent accent wash via the `--color-highlight` token), `__underline__` (accent underline), and `[label](https://url)` external links. Tokens don't nest. Use backticks for every property/function/value name in prose, `==...==` for the one line per section a skimmer must not miss, and use both sparingly — if everything is highlighted, nothing is.

**Styling**: Tailwind v4 via `@tailwindcss/vite`, imported in `src/styles/global.css` with `@import "tailwindcss"`. Theme is still toggled manually rather than following OS preference only — see `ThemeToggle.tsx` (persists to `localStorage`, guarded with try/catch since storage can throw in sandboxed preview contexts) and the inline theme-init script in `index.html` (applies the class before first paint to avoid a flash of unstyled theme) — but the mechanism is the CSS `light-dark()` function, not Tailwind's `dark:` variant. `ThemeToggle` adds/removes a `.dark` class on `<html>`; in `global.css`, `:root`/`:root.dark` use that class only to flip the `color-scheme` property (`light`/`dark`), and every themed color is a single `@theme` custom property defined with `light-dark(lightValue, darkValue)` (`--color-fg`, `--color-fg-invert`, `--color-body`, `--color-muted`, `--color-divider`, `--color-border`, `--color-bg`, `--color-surface`, plus `--color-accent`/`--color-accent-hover` and the translucent `--color-highlight`). Components use one class per color (e.g. `text-fg`, `border-border`, `bg-surface`) instead of a `dark:`-prefixed pair — **do not reintroduce `dark:` utility classes; add a new `light-dark()` token in `global.css` and reuse it instead.** The visual language is a restrained, high-contrast, near-black/near-white palette with no shadows — see the `design-principles` rules article in `articles.ts` for the explicit rules this site follows, and follow them for any new UI.
